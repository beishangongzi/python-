...
====================================================
经过测试发现
如果单独执行add函数，执行一次需要1.5s
如果开启多个线程，三个add函数执行完毕，需要4.5s,这就相当于是串行执行的，或者说三个add函数是并发执行的
如果开启多个进程，三个add函数执行完毕，需要的时间是1.6s,这是真正使用了多个cpu,是并行，所以省时间
...
也就是说，你可以编写一个拥有多个线程或者进程的并发程序，
但如果没有多核处理器来执行这个程序，那么就不能以并行方式来运行代码。
